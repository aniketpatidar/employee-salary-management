...
class PipelineTest(unittest.TestCase):
    def test_clean_address_is_not_flagged_and_reports_leftover_text(self):
        result = parse("Harbour View 1 Flat no.104 A Cuffe Parade, Mumbai 400005")
        self.assertEqual(result.values, {"pin_code": "400005", "city": "Mumbai", "area": "Cuffe Parade"})
        self.assertEqual(flag_reasons(result), [])
        self.assertEqual(result.unparsed, "Harbour View 1 Flat no.104 A")

    def test_blank_required_fields_are_named(self):
        self.assertEqual(flag_reasons(parse("47, mumbai")), ["area missing", "pincode missing"])

    def test_a_conflict_is_reported_instead_of_also_calling_the_field_missing(self):
        reasons = flag_reasons(parse("Flat 1, Powai 400076, Worli 400018, Mumbai"))
