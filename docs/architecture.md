...
**File structure**:
- Backend: `app/models/employee.rb` (enums, `COUNTRY_CURRENCY_MAP`, validations, `Employee.filtered(filters)` over an allowlisted `FILTER_FIELDS`, plus `Employee.pay_insights(filters, breakdown)` class method), `app/models/user.rb`, `app/models/session.rb`, `app/controllers/employees_controller.rb`, `app/controllers/pay_insights_controller.rb`, `app/controllers/sessions_controller.rb`, `app/controllers/passwords_controller.rb`, `app/controllers/concerns/authentication.rb` (from the generator), `db/seeds.rb`
